...
public class AkkaConfigPropertySourceAdapter {
    ...
    /**
     * Flattens the properties of an environment into a Map.
     */
    public static Map<String, String> flattenEnvironment(ConfigurableEnvironment environment) {
        final Map<String, String> flattened = environment.getPropertySources()
                .stream()
                .filter(s -> s instanceof EnumerablePropertySource)
                .map(s -> (EnumerablePropertySource) s)
                .collect(
                        HashMap::new,
                        (m, s) -> Arrays.stream(s.getPropertyNames())
                                .forEach(k -> { m.put(k, environment.getProperty(k)); }),
                        HashMap::putAll
                );
